...
<div class="container mx-auto px-6 py-8">
    <div class="flex justify-between items-center mb-6">
        <h3 class="text-gray-700 text-3xl font-medium">ویرایش آلبوم</h3>
        ...
        <form action="{{ route('admin.albums.update', $album) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mt-6">
                ...
                <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md"
                    ...
                    <div class="space-y-1 text-center">
                        ...
                        <div id="cover-preview" class="mt-4 {{ $album->cover_image ? '' : 'hidden' }}">
                            <img src="{{ $album->cover_image ? asset('storage/' . $album->cover_image) : '' }}" 
                                alt="پیش‌نمایش" class="mx-auto h-32 w-32 object-cover rounded-lg">
                        </div>
                    </div>
                </div>
                @error('cover_image')
            ...
            <div class="mt-6">
                ...
                <textarea name="description" id="description" rows="4"
                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('description') border-red-500 @enderror">{{ old('description', $album->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
